...
$(document).ready(function(){
	...
	function countdown () {
	  var now = moment(), // get the current moment
		then = moment([2020, 2, 7]); // target date
  
	  // get the difference from now to then in ms once
	  ...
	  var hours = Math.floor(moment.duration(then.diff(now, 'milliseconds', true)).asHours());
	  then = then.subtract(hours, 'hours');
  
	  var minutes = Math.floor(moment.duration(then.diff(now, 'milliseconds', true)).asMinutes());
	  then = then.subtract(minutes, 'minutes');
  
	  var seconds = Math.floor(moment.duration(then.diff(now, 'milliseconds', true)).asSeconds());
  
	  // concatonate the variables
	  var diff = '<div class="num">' + days + ' <span class="text"> Days Left</span></div>';
	  $('#daysLeft').html(diff);
	}
  });
